...
bool ModuleMap1::Init()
{
	...
	Map1 = App->textures->Load("Tilemap2.png");
    Road = App->textures->Load("Roads.png");
	Buildings = App->textures->Load("Buildings.png");
...
update_status ModuleMap1::Update()
{
	...
	{
		snprintf(App->player2->highscore_text, 20, "HI-SCORE");
		snprintf(App->player2->highscoret, 20, "   %7d", App->player2->hiscore);
		snprintf(App->player2->lastscore_text, 20, "2UP");
		snprintf(App->player2->lastscoret, 20, "   %7d", App->player2->lastscore);
		App->player2->allowhiscore = false;
	}

	if (App->player->allowhiscore == true)
	{
		snprintf(App->player->highscore_text, 20, "HI-SCORE");
		snprintf(App->player->highscoret, 20, "%7d", App->player->hiscore);
		snprintf(App->player->lastscore_text, 20, "GODMODE");
		snprintf(App->player->lastscoret, 20, "%7d", App->player->lastscore);
